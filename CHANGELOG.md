...
- **Tablo kaydırılınca satırlar başlığın içinden görünmüyor.** Başlık satırı bütün olarak sabitlendiği için bazı tarayıcılarda (özellikle Safari) satırlar başlığın arkasından sızıyordu. Başlık hücreleri artık tek tek sabitleniyor.
...
- Satırda artık **"IP'leri ve zamanları log'dan getir"** düğmesi var: tıklayınca "Log'da ara" bölümü o isteği yöntemi, tam adresi ve yanıt koduyla arıyor. Arama log dosyasını doğrudan okuduğu için bu süre sınırına takılmıyor.
- Aramaya **tam adres** seçeneği eklendi. Eskiden arama yalnızca "yolun içinde geçen" metne bakıyordu; "/" için arama her şeyi buluyordu. Düğmeden gelen aramalar tam adresle yapılıyor ve bu formun altında yazıyor; tek tıkla normal aramaya dönülebiliyor.
...
- **Yöntem ve yol her yerde alt alta.** Yan yana yazıldığında kısa yollar tek satıra sığıyor ("GET /WebResource.axd"), uzun yollar kırılıp yöntem yukarıda yalnız kalıyordu; satırlar birbirine benzemiyordu. Artık hepsi aynı düzende: üstte küçük yöntem etiketi (GET, POST), altında yol, varsa en altta backend.
- Aynı düzen dört yerde birden uygulandı: en çok istenen adresler, hangi adres ne döndürüyor, engellenen ve karşılıksız kalan istekler, arama sonuçları.
- Uzun yollar artık "/" işaretlerinden kırılıyor ("…asmx/" ve alt satırda "GetUniformBodySize"); eskiden kelime ortasından bölünebiliyordu. Telefonda dar tablolarda yer yetmediğinde tarayıcı yine kelime içinden kırar.
...
- Tek kodlu satırlar sade yazı ("tümü ● 200 başarılı"); kutulu etiket yalnızca farklı kodlar alan satırlarda. Yirmi satırın hepsinde aynı kutu göz yoruyordu ve asıl önemli olan istisnaları boğuyordu; artık onlar göze çarpıyor.
- Telefonda sayfa yana taşmıyor: dar ekranda "Cloudflare" ve kodların anlamı gizleniyor (üzerine gelince görünüyor).
...
- Sunucuların hata oranları birbirine yakınsa bulgu bunu da söylüyor: sorun büyük ihtimalle tek bir sunucuda değil, hepsinin kullandığı ortak bir yerde.
- Backend ayrıntısındaki hata özetinde de aynı ayrım yapıldı ("sunuculardan dönenlerin %43 kadarı" ifadesi aynı karışıklığı yaratıyordu).
