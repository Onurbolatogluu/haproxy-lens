...
Kurulumdaki tek satırın aynısını çalıştırmak yeterli: her seferinde en son sürümü indirir, betik de önceki kurulumu görüp üzerine yazar. Bütün ayarların korunur (port, adres, erişim listesi, log kaynağı, saklama süreleri, bellek bütçesi ve tavanı); yalnızca değiştirmek istediklerini vermen yeterli.
...
Servisi, program dosyasını, saklanan geçmişi ve `haproxy-lens` sistem kullanıcısını siler, geride bir şey kalmadığını kontrol eder. Kurulumdaki iki doğrulamayı burada da yapar. Paketin klasörü artık yoksa aynı paketi tekrar indirip içindeki `uninstall.sh`'ı çalıştırabilirsin.
...
- **HAProxy'nin yeniden başlaması:** HAProxy yeniden başlarsa kendi birikmiş sayaçları ("toplam" diye gösterilenler) sıfırlanır. Grafikler ve oranlar ayrı tutulduğu için önceki trafiği göstermeye devam eder.
...
Ajan her 2 saniyede stats'ı okur; saniyelik değerleri iki ölçüm arasındaki sayaç farkından hesaplar. Log satırlarını dakikalık özetlere çevirip saklama süresi boyunca (varsayılan 24 saat) tutar. Arayüz programın içine gömülüdür, ayrıca bir web sunucusu gerekmez.
